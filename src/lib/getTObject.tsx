 ...
 */
export function getTObject(namespace: string, hookParams: Params = {}, locale?: string) {
  ...
  const t = (() => {
    ...
    return async function tObject(keyPath: string, callParams: Params = {}): Promise<TObjectType> {
      const finalParams = { ...hookParams, ...callParams };
      const fullPath = namespace ? `${namespace}.${keyPath}` : keyPath;

      const parts = fullPath.split('.');
      let value: any = await getMessages({ locale });
